...
#lawson_stacked
#def f(row):
#    if row['shift_code'] == 'A0':
#        val = pd.DatetimeIndex(lawson['cal_day']) + pd.DateOffset(1)
#    else:
#        val = lawson['cal_day']
#    return val
...
#lawson2.apply(f, axis=1)
#lawson['shift_end'] = np.where(lawson['shift_code'] == 'A0', lawson['cal_day'] + pd.DateOffset(-1) , lawson['end_time'])

#####################################################
#pd.to_datetime('1970-01-01').value
#lawson['shift_end2'] = pd.to_datetime(lawson['cal_day']).value
#lawson['shift_end'] = pd.DatetimeIndex(lawson['cal_day']) + pd.DateOffset(-1)
...
#lawson_stacked['cal_day'].dt(dayofweek)
...
#        n_day = (pd.DatetimeIndex(lawson['cal_day']) + pd.DateOffset(1))
#        next_days.append(pd.DatetimeIndex(lawson['cal_day']) + pd.DateOffset(1))
#        print('true')
#        print(n_day)
#    else:
#        # Append a failing grade
#        next_days.append(lawson['cal_day'])
#        print('false')
...
#lawson['shift_end'] = np.where(lawson.shift_code == 'A0', pd.DatetimeIndex(lawson['cal_day'] + pd.DateOffset(1), False))
#lawson.loc[lawson['shift_end']] = lawson.shift_code.apply(lambda x: pd.DatetimeIndex(lawson2['cal_day']) + pd.DateOffset(1) if x == 'A0' else lawson_stacked['cal_day'])
#gapminder.head()
#lawson2.loc[lawson2['shift_code'] == 'A0', 'shift_end'] = pd.DatetimeIndex(lawson2['cal_day']) + pd.DateOffset(1) 
#lawson_stacked.loc[lawson_stacked['shift_code'] == 'A4', 'shift_end'] = lawson_stacked['cal_day']
